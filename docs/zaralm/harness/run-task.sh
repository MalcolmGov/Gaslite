...
set -u
SP=/tmp/claude-0/-home-user-Gaslite/83e89079-b6fe-5121-85ff-cc06d993ce0b/scratchpad
LABEL=$1; shift; TASK="$*"
...
export PATH="/home/user/malcolmgov/protea/.venv/bin:$PATH"
...
const f=fs.readdirSync(dir,{recursive:true}).map(p=>path.join(dir,p)).find(p=>p.endsWith(".jsonl.zstd"));
if(!f){console.log("no session log");process.exit(0);}
fs.writeFileSync(process.argv[2],zlib.zstdDecompressSync(fs.readFileSync(f)).toString("utf8"));
console.log("session log ->",process.argv[2]);
' "$DSH_HOME/sessions" "$SP/logs/session-$LABEL.jsonl"
