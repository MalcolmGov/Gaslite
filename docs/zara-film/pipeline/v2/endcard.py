...
Bm.save("card_B.png")
Cm=Image.new("RGBA",(W,H),(0,0,0,0)); d=ImageDraw.Draw(Cm)
def spaced2(d,y,txt,f,fill,sp):
    cw=[d.textlength(ch,font=f) for ch in txt]; x=(W-(sum(cw)+sp*(len(txt)-1)))/2
    for ch,w in zip(txt,cw): d.text((x,y),ch,font=f,fill=fill); x+=w+sp
spaced2(d,238,"THE AGENTIC MARKETPLACE FOR BUSINESS",F_URL,CY+(255,),6)
spaced2(d,930,"PLUG-AND-PLAY AGENTS   ·   NO CODE   ·   SECURE   ·   COMPLIANT   ·   FIVE CONTINENTS",pick(["Montserrat-Medium.ttf","Montserrat-ExtraBold.ttf"],22),MU+(255,),3)
Cm.save("card_C.png")
P=Image.new("RGBA",(W,H),BG+(255,)); P.alpha_composite(A); P.alpha_composite(Bm); P.alpha_composite(Cm); P.convert("RGB").save("endcard_preview.jpg",quality=90)
